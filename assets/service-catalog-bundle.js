...
const CollapsibleDescription = ({ title, description, }) => {
    ...
    const [showToggleButton, setShowToggleButton] = reactExports.useState(false);
    ...
    return (jsxRuntimeExports.jsxs(DescriptionWrapper, { children: [jsxRuntimeExports.jsx(ItemTitle, { tag: "h1", children: title }), jsxRuntimeExports.jsx(CollapsibleText, { expanded: isExpanded, ref: descriptionRef, children: description }), showToggleButton && (jsxRuntimeExports.jsxs(ToggleButton, { isLink: true, onClick: toggleDescription, children: [isExpanded
...
function ServiceCatalogItemPage({ serviceCatalogItemId, baseLocale, hasAtMentions, userRole, organizations, userId, brandId, }) {
    ...
    reactExports.useEffect(() => {
        async function fetchServiceCatalogItem() {
            try {
                const response = await fetch(`/api/v2/help_center/service_catalog/items/${serviceCatalogItemId}`);
                const data = await response.json();
                if (response.ok) {
